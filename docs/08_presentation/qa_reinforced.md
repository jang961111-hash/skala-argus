...
> - ✅ **확정 수치**: pytest **30 passed** · E2E **64/0** · 빌드 **223.58 kB**(gzip 81.73) · 118 모듈. 원본 로그 3회 실행 동일, 발표자 대조 완료.
...
검증은 `bash scripts/e2e_live.sh`로 합니다. 이건 pytest와 다릅니다. **pytest는 TestClient라 네트워크를 안 타지만, 이 스크립트는 실제 uvicorn을 포트에 띄우고 curl로 HTTP를 때립니다. 데모와 같은 조건입니다.** **64건 전부 통과, 실패 0건입니다.** 백엔드 테스트도 30개 전부 통과했습니다.
...
**⚠️ 정직성 2가지**: ①**64건 중 1건은 판정이 항상 통과하는 스크립트 결함이 있다**(사고 로그 I-12). 유효 판정 63건. 로그의 `PASS … FAIL:[...]` 줄을 지적받으면 숨기지 말고 그대로 설명한다 — 사고 로그에 원인·증거·조치안이 다 있다. ②여기에 **Supabase는 포함되지 않는다.** 물으면 "스키마는 PostgreSQL DDL로 썼고 실행 검증은 못 했습니다"라고 말한다.
