...
| R7 | 데모 중 데이터 오염 (리허설로 요청이 APPROVED 상태로 남음) | 시나리오 재현 불가 | 목록에 중복 요청 | 데이터 리셋 명령(`rm replaceflow.db` 후 재기동 → 시드) 을 리허설 직후 실행, 체크리스트에 포함 | 신서현 |
