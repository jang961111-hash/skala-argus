...
| 문서 버전 | **v3.0 (2026-09-03)** — 팀 노션 「API 명세서 v1.0」+「팀 데이터 모델 정의서 v3.0」+「WRA 화면정의서 v2.0」 원문 기준 |
| 서비스명 | **Argus** (D-11, 팀장 확정 09-03). `ReplaceFlow`(저장소·기획서 가칭)와 데이터 모델 정의서 제목이었던 옛 명칭은 전부 폐기 |
...
| v3.0 | 09-03 | 팀 노션 「API 명세서 v1.0」+「팀 데이터 모델 정의서 v3.0」+「WRA 화면정의서 v2.0」을 그대로 반영. 에이전트 코드 SPEC/LEGAL/SAFETY_DOC→`A1`/`A2`/`A3`, 상태값 8종→6종(`DRAFT`·`AI_RUNNING`·`AI_DONE`·`PENDING`·`APPROVED`·`REJECTED`), 결과 구조를 에이전트별 고유 스키마에서 통일 구조(`items[]`/`documents[]`)로 단순화, API 15개 확정, 법령·설비·부품 마스터는 전부 Phase 2. 근거: `docs/CONTRACT.md` v3.0(255줄) |
| **서비스명 확정** | **09-03** | **`ReplaceFlow`(가칭) → `Argus` 확정(D-11, 팀장 결정). 저장소·전 문서 87파일 자동 치환** |
...
서비스명: **Argus** (설비 부품 교체 승인 에이전트, D-11 확정)
