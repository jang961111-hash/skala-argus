...
> ⚠️ **서비스명**: 발표에서는 `Argus` 로 통일한다. ERD 문서의 `FixGuide` 는 내부 작업명이다.
...
| **서비스명이 문서마다 다른데?** | ERD 문서의 `FixGuide`는 데이터 모델을 쓸 때의 내부 작업명이고 **서비스명은 `Argus`다. 문서 제목을 못 맞춘 건 저희 실수다** — 변명 없이 한 문장으로 끝낸다 | CONTRACT §0 각주 |
