...
| 항목 | 근거 |
|---|---|
| BE 구현 (인증·사진·결과편집·승인) | `pytest` **30 passed** · `e2e_live_v3.sh` **72/0** |
| FE 9화면 + 실연동 | `npm run build` **223.88 kB**(gzip 81.98) · argus_ 캡처 9장 |
| ERD·API·Postman·에러코드 | 8테이블 · 12 paths/15 ops · 16요청/67예시 · 23종 |
| 명칭 통일 | **Argus** — 87파일 236곳, 다이어그램 7종 재렌더, 시드 `@argus.test` |
| Git 이력 | `develop` + `feature/*` **9개**, `--no-ff` 병합 10건+, 33커밋 push 완료 |
...
| 6 | **리허설 3회** (13:00 · 14:00 · 14:40) | 전원 | 각 20분 | 9/4 14:40 |
