...
# §4. 남은 일 — **2026-09-03 16:37 기준 재작성**

프로젝트명 **`Argus` 확정**. 구현·문서·검증·커밋은 **끝났다.** 남은 건 대부분 사람이 해야 하는 것이다.

## 4-0. ✅ 완료로 이동 (더 이상 추적 안 함)

| 항목 | 근거 |
|---|---|
| BE 구현 (인증·사진·결과편집·승인) | `pytest` **30 passed** · `e2e_live_v3.sh` **72/0** (`20260903_1659` 직접 실행) |
| FE 9화면 + 실연동 | `npm run build` **223.88 kB**(gzip 81.98) · argus_ 캡처 9장 |
| ERD·API·Postman·에러코드 | 8테이블 · 12 paths/15 ops · 16요청/67예시 · 23종 |
| 명칭 통일 | **Argus** — 87파일 236곳 · 다이어그램 7종 재렌더 · 시드 `@argus.test` · **레포 `skala-argus` · 로컬 `~/projects/argus`** |
| Git 이력 | **커밋 37 · 브랜치 12**(`main`+`develop`+`feature/*` 10) · 파일 171 · `--no-ff` 병합 10건+ · **미커밋 0**, push 완료 |
| 기록 체계 | `10_project_record/` 원본 로그·캡처·인벤토리 |

## 4-1. 남은 일 — 전부 내일 오전에 몰려 있다

| # | 항목 | 담당 | 소요 | 마감 |
|---|---|---|---|---|
| 1 | **발표 슬라이드 실물 제작** (Argus 명칭·최종 실측치 반영) | 은태현 | 120분 | 9/4 11:00 |
| 2 | └ N:M 0개 방어 슬라이드 (1:N 8개 + 부재 사유 + `erd_phase2.svg`) | 은태현 | 30분 | 9/4 11:00 |
| 3 | └ Git 브랜치 그래프 캡처 (`git log --graph --merges`) — Contributors 캡처 **대신** | 은태현 | 10분 | 9/4 11:00 |
| 4 | **회고 §3 R&R별 5명 + §4 KPT** | 전원 | 각 10분 | 9/4 10:00 |
| 5 | **타 조 Peer Review 질의 1개 확정** | 전원 | 10분 | 9/4 12:00 |
| 6 | **리허설 3회** (13:00 · 14:00 · 14:40) — 런북 `docs/09_qa/demo_runbook.md` | 전원 | 각 20분 | 9/4 14:40 |
| 7 | 데모 대본 최종 (클라이맥스 = 폴링 3종 완료 → 결과 편집 → 승인) | 장병헌 | 30분 | 9/4 12:00 |
| 8 | `WRA_C_01` 회원가입 캡처 1장 (9화면 중 유일 미캡처) | 문승은 | 5분 | 9/4 10:00 |

## 4-2. **사람이 직접 해야만 하는 것** (에이전트 대행 불가)

| 담당 | 항목 | 상태 | 마감 |
|---|---|---|---|
| 팀장 | **Stitch 화면 제작** | 진행 중 — 완료분만 반영 | 9/4 11:00 |
| 팀장 | **Supabase 프로젝트 생성 + DDL·seed 실행** | **미검증.** 런북 `06_erd/supabase_apply.md` 준비됨 | 9/4 11:00 |
| 은태현 | 발표 슬라이드 조판 | 미착수 | 9/4 11:00 |
| 전원 | 회고 각자 2줄 | 미작성 | 9/4 10:00 |
| 전원 | 리허설 → 발표 | 미실시 | 9/4 15:00 |
| 신서현 | `.omc` 이력 purge | 미완 (채점 무관 — 3순위) | 9/4 13:00 |

## 4-3. 절단 순서

1. **절대 사수**: 슬라이드 · 리허설 3회 · 회고
2. 다음: Supabase 실행 · Stitch 완료분 · C_01 캡처
3. **먼저 버림**: `.omc` purge(채점 무관) · Postman Mock 퍼블리시

> 구현이 끝났으므로 **내일의 리스크는 기술이 아니라 발표 준비**다. 슬라이드와 리허설에 시간을 몰아준다.
