...
| PATCH | /work-requests/{id}/complete | 200 `WorkRequest`(status:DONE) / 409(APPROVED 아니면) / 404 — 승인 이후 "작업 완료 처리" (BE·`test_flow.py`·`scripts/e2e_live.sh`엔 있었으나 표에 누락돼 있었음, 2026-09-03 추가) |
...
> `approval_required_by`: openapi.yaml에는 `UserRole` enum(`ENGINEER|SAFETY_MANAGER|BUYER|ADMIN`)으로 명세돼 있으나, BE(`backend/app/schemas/agent_run.py`)는 이를 plain `str`로 완화 구현했고 값은 항상 `SAFETY_MANAGER` 고정이다. openapi 스키마는 의도된 명세로 그대로 두고(수정은 BE 담당 소관), 이 표기를 실제 계약으로 삼는다. (2026-09-03)
